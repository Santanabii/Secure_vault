...
class LoginWindow:
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.root, text="Secure Password Manager", 
        ...
        self.password_entry = ctk.CTkEntry(self.root, width=340, height=45, 
                                         placeholder_text="Enter master password", show="*")
        self.password_entry.pack(pady=(5, 20))

        # Buttons
        ctk.CTkButton(self.root, text="Login", width=340, height=50, 
                     font=ctk.CTkFont(size=16, weight="bold"),
                     command=self.login).pack(pady=10)

        ctk.CTkButton(self.root, text="Create New Account", width=340, height=40,
                     fg_color="transparent", border_width=2,
                     command=self.set_master_password).pack(pady=8)

        # Password Requirements
        ctk.CTkLabel(self.root, text="Master Password must be 8+ characters\n"
                                    "with letters, numbers & symbols", 
    ...
    def login(self):
        ...
        if not username or not password:
            messagebox.showwarning("Input Error", "Please enter username and master password")
            return
